...
import { createClient, updateClient } from '@/lib/api/catalog'
import type { ClientDetail } from '@/types/catalog'
...
export function AddClientModal({ client, onClose, onSuccess }: Props) {
  const isEdit = !!client
  ...
  return (
    <Modal title={isEdit ? 'Edit client' : 'Add client'} onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Field label="Cafe / business name *" error={errors.name}>
          <input value={form.name} onChange={e => set('name', e.target.value)}
            className={cx(errors.name)} placeholder="Blue Bottle Kailua" />
        </Field>
